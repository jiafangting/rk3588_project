 ...
 * - 查询频率默认 2 秒一次。
 */
#define VISION_SOCKET_PATH "/tmp/vision.sock"
...
void *thread_vision(void *arg)
{
    ...
    while (g_running) {
        ...
        char buffer[4096];
        ssize_t nread;
        const char *request = "{\"cmd\":\"query_status\"}\n";

        memset(&addr, 0, sizeof(addr));
        addr.sun_family = AF_UNIX;
        snprintf(addr.sun_path, sizeof(addr.sun_path), "%s", VISION_SOCKET_PATH);
        ...
        if (connect(sockfd, (struct sockaddr *)&addr, sizeof(addr)) != 0) {
            printf("[VISION] 连接 /tmp/vision.sock 失败，等待下次重试\n");
            pthread_mutex_lock(&state->lock);
            snprintf(state->vision_status, sizeof(state->vision_status), "UNKNOWN");
            pthread_mutex_unlock(&state->lock);
            close(sockfd);
